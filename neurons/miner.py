...
class Miner(BaseMinerNeuron):
    ...
    def __init__(self, config=None):
        ...
        env_model_name = str(os.getenv("POKER44_MODEL_NAME", "")).strip()
        if env_model_name:
            model_name_default = env_model_name
        elif self.predictor is not None and self.batch_calibration not in {
            "",
            "none",
            "model_default",
            "off",
        ...
        self.model_manifest = build_local_model_manifest(
            ...
            defaults={
                ...
                "model_version": (
                    str(os.getenv("POKER44_MODEL_VERSION", "")).strip()
                    or str(model_metadata.get("model_version", "")).strip()
                    or "1"
                    if self.predictor is not None
                    else "2"
                ),
...
_BUILD_FINGERPRINT_E1 = "d7e4c2596d0112b13cb18475"
_BUILD_SALT_E1 = "3d2d1dac1d95705c"
